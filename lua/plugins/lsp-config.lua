...
return {
  ...
  -- Bridge Mason ↔ LSPConfig (modern behavior on 0.11+)
  {
    ...
    opts = {
      ...
      ensure_installed = {
        "lua_ls", "ts_ls", "pyright", "eslint", "html", "cssls",
  ...
  -- Server configs (modern Neovim API + lspconfig’s configs in rtp)
  {
    ...
    config = function()
      ...
      local function on_attach(_, bufnr)
        ...
        buf_keymap("n", "<C-k>", vim.lsp.buf.signature_help,  "LSP: signature help")
        ...
        buf_keymap("n", "[d", function() vim.diagnostic.jump({ count = -1 }) end, "Diag: previous")
        buf_keymap("n", "]d", function() vim.diagnostic.jump({ count =  1 }) end, "Diag: next")
